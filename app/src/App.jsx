...
function TransposeDialog({song,onClose,onSave,onKeysResolved,embedded=false,homeEmbedded=false}) {
  ...
  useEffect(()=>{analyzeSongChords(song.id).then((data)=>{const recognized=resolveEditorSourceKey(song, data.key || data.sourceKey || data.originalKey)||inferKeyFromLeadsheet(data.text);setOriginalText(data.text);if(recognized){setSourceKey(recognized);setTargetKey(recognized);onKeysResolved?.({sourceKey:recognized,originalKey:recognized,key:recognized})}setNeedsReview(Boolean(data.needsReview)||!recognized);const tempo=data.text.match(/TEMPO:\s*(\d{2,3})\s*BPM/i);if(tempo)setBpm(Math.min(240,Math.max(40,Number(tempo[1]))));setLoading(false)}).catch((caught)=>{setError(caught.message);setLoading(false)})},[song.id])

...
function TransposeDialog({song,onClose,onSave,embedded=false,homeEmbedded=false}) {
  const { t } = useI18n()
  const initialKey=germanKeys.includes(song.sourceKey||song.key)?(song.sourceKey||song.key):'D';const [text,setText]=useState('');const [sourceKey]=useState(initialKey);const [targetKey,setTargetKey]=useState(initialKey);const [currentKey,setCurrentKey]=useState(initialKey);const [error,setError]=useState('');const [loading,setLoading]=useState(true);const [saving,setSaving]=useState(false);const [saved,setSaved]=useState('');const [needsReview,setNeedsReview]=useState(false);const [fontSize,setFontSize]=useState(16);const [columns,setColumns]=useState(1);const [autoScroll,setAutoScroll]=useState(false);const [bpm,setBpm]=useState(120);const [cajonOn,setCajonOn]=useState(false);const audioContextRef=useRef(null);const [view,setView]=useState(hasSongPdf(song)?'original':'edited')
  useEffect(()=>{analyzeSongChords(song.id).then((data)=>{setText(data.text);setNeedsReview(Boolean(data.needsReview));const tempo=data.text.match(/TEMPO:\s*(\d{2,3})\s*BPM/i);if(tempo)setBpm(Math.min(240,Math.max(40,Number(tempo[1]))));setLoading(false)}).catch((caught)=>{setError(caught.message);setLoading(false)})},[song.id])

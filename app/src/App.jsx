...
import './App.css'
import './extra.css'
import { analyzeSongChords, deleteSong, getImportedSongs, getSongOriginalSnapshot, getSongVariants, hasSongPdf, openSongChart, openSongPdf, previewScanPdf, saveImportedSongs, saveScanImport, saveSongVariant, songChartUrl, songPdfUrl, updateSong, resolveSongCover, songCoverPath } from './songStore'
...
function Footer() {
  ...
  const donationUrl='https://www.paypal.com/donate?business=eduardwiebe77%40gmail.com&no_recurring=0&item_name=Lyruma+Songbook+Open+Source+Entwicklung&currency_code=EUR'
...
function HomePage({songs, setSongs, sets, openImport, openSetDialog, navigate}) {
  ...
  return <><Header title={t('home.title')} subtitle={t('home.subtitle')}/>
    ...
    {selectedSong?<TransposeDialog embedded homeEmbedded song={selectedSong} onClose={()=>setSelectedSongId('')} onKeysResolved={(keys)=>setSongs((current)=>current.map((item)=>item.id===selectedSong.id?{...item,...keys}:item))} onSave={async(values)=>{const variant=await saveSongVariant(selectedSong.id,values);setSongs((current)=>current.map((item)=>item.id===selectedSong.id?{...item,key:variant.targetKey,sourceKey:variant.sourceKey,originalKey:variant.sourceKey,preferredKey:variant.targetKey,variantKeys:Array.from(new Set([variant.targetKey,...(item.variantKeys||[])]))}:item));return variant}}/>:<><section className="home-section"><div className="home-section-head"><div><p className="eyebrow">{t('home.library')}</p><h2>{query?t('home.searchResults', { query }):t('home.openSongs')}</h2></div><button className="text-button" onClick={openImport}><Plus size={17}/>{t('home.addSong')}</button></div>{shown.length?<div className="song-tile-row">{shown.map((song,index)=><button className="song-tile" key={song.id} onClick={()=>setSelectedSongId(song.id)}><span className={`song-cover cover-tone-${index%6}${song.hasCover?' has-cover':''}`}>{song.hasCover?<AuthorizedImg className="song-cover-image" path={songCoverPath(song)} alt=""/>:null}<span className="song-cover-title">{song.title}</span><Music2 className="song-cover-note" size={22} aria-hidden="true"/><i className="song-cover-play"><Play size={17}/></i></span><span className="song-tile-copy"><span className="song-rank">{index+1}</span>{(song.preferredKey||song.key)&&<span className="song-key">{t('home.key', { key: song.preferredKey||song.key })}</span>}<strong>{song.title}</strong><small>{song.artist||t('brand.songbook')}</small><span className="tile-open">{t('home.openInEditor')} <ChevronRight size={15}/></span></span></button>)}{!query&&songs.length>12&&<button className="song-tile more-tile" onClick={()=>navigate('/songs')}><span className="more-cover"><Plus size={30}/></span><span className="song-tile-copy"><span className="song-rank">12+</span><strong>{t('home.allSongs')}</strong><small>{t('home.fullLibrary')}</small><span className="tile-open">{t('home.toLibrary')} <ChevronRight size={15}/></span></span></button>}</div>:<div className="empty-state small"><Search size={30}/><h3>{t('home.noSongTitle')}</h3><p>{t('home.noSongText')}</p></div>}</section>
...
function SongEditorRoute({songs,setSongs,navigate}) {
  ...
  return <TransposeDialog embedded song={song} onClose={()=>navigate('/songs')} onKeysResolved={(keys)=>setSongs((current)=>current.map((item)=>item.id===song.id?{...item,...keys}:item))} onSave={async(values)=>{const variant=await saveSongVariant(song.id,values);setSongs((current)=>current.map((item)=>item.id===song.id?{...item,key:variant.targetKey,sourceKey:variant.sourceKey,originalKey:variant.sourceKey,preferredKey:variant.targetKey,variantKeys:Array.from(new Set([variant.targetKey,...(item.variantKeys||[])]))}:item));return variant}}/>
}

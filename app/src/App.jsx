...
import { LeadSheetOsmd } from './LeadSheetOsmd'
...
import { preferSongTitle } from '../../lib/songMetadata.mjs'
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!original.ok) {
          if (fromMeta == null) { setBpm(null); setBpmInput('') }
          return
        }
        ...
        if (active && fromMeta == null) { setBpm(null); setBpmInput('') }
      }
    })()
    return () => { active = false }
...
function TransposeDialog({song,onClose,onSave,onKeysResolved,embedded=false,homeEmbedded=false}) {
  const { t, locale } = useI18n()
  const [originalText,setOriginalText]=useState('');const [overlayText,setOverlayText]=useState('');const [sourceKey,setSourceKey]=useState('');const [targetKey,setTargetKey]=useState('');const [snapshotVerified,setSnapshotVerified]=useState(false);const [error,setError]=useState('');const [loading,setLoading]=useState(true);const [saving,setSaving]=useState(false);const [saved,setSaved]=useState('');const [needsReview,setNeedsReview]=useState(false);const [reanalyzing,setReanalyzing]=useState(false);const [reloadToken,setReloadToken]=useState(0);const [fontSize,setFontSize]=useState(()=>Math.min(28,Math.max(11,Number(song.sheetFontSize)||16)));const [columns,setColumns]=useState(()=>Number(song.sheetColumns)===2?2:1);const [autoScroll,setAutoScroll]=useState(false);const [bpm,setBpm]=useState(null);const [bpmInput,setBpmInput]=useState('');const [cajonOn,setCajonOn]=useState(false);const [view,setView]=useState(hasSongPdf(song)?'original':'chords');const [simplifyChords,setSimplifyChords]=useState(()=>readSimplifyChordsPref(locale));const [lyricsOnly,setLyricsOnly]=useState(()=>{try{const s=localStorage.getItem('songbook-lyrics-only');return s==='1'||s==='true'}catch{return false}});const [youtubeUrl,setYoutubeUrl]=useState(song.youtubeUrl||'');const [youtubeResolving,setYoutubeResolving]=useState(false);const [tunerOpen,setTunerOpen]=useState(false)
  ...
  return <div className={`${embedded?'song-editor-page':'modal-backdrop'}${homeEmbedded?' home-song-editor':''}`} onMouseDown={(event)=>!embedded&&event.target===event.currentTarget&&close()}>{embedded&&<button className="back-button editor-back" onClick={close}><ChevronLeft size={18}/>{t('songs.toLibrary')}</button>}<section className={embedded?'song-editor-surface':'modal modal-wide transpose-modal'}>{loading?<div className="analysis-loading"><Music2 size={30}/><strong>{t('songs.preparing')}</strong></div>:error?<div className="form-error analysis-error">{error}</div>:<><div className="editor-view-switch"><button className={view==='original'?'active':''} onClick={()=>setView('original')} disabled={!hasSongPdf(song)}>{t('songs.originalPdf')}</button><button className={view==='chords'||view==='edited'?'active':''} onClick={()=>setView('chords')}>{t('songs.editKey')}</button><button className={view==='leadsheet'?'active':''} onClick={()=>setView('leadsheet')}>{t('songs.viewLeadsheet')}</button><button type="button" className="youtube-rehearsal" onClick={openYoutubeRehearsal} disabled={youtubeResolving&&!youtubeUrl} title={t('songs.youtubeRehearsalHint')}>{youtubeResolving&&!youtubeUrl?t('songs.youtubeResolving'):t('songs.youtubeRehearsal')}</button><button type="button" className="guitar-tuner-btn" onClick={()=>setTunerOpen(true)} title={t('songs.tunerHint')}>{t('songs.tuner')}</button><span>{view==='original'?t('songs.originalHint'):view==='leadsheet'?t('songs.leadsheetHint'):t('songs.editableHint')}</span></div>{needsReview&&(view==='chords'||view==='edited')&&<div className="analysis-quality-warn" role="status"><p>{t('songs.snapshotReviewRequired')}</p>{!hasSongPdf(song)&&<p>{t('songs.reuploadHint')}</p>}</div>}{(view==='chords'||view==='edited')&&hasSongPdf(song)&&<div className="editor-reanalyze"><button type="button" className="text-button" disabled={reanalyzing} title={t('songs.reanalyzeHint')} onClick={reanalyzeFromPdf}>{reanalyzing?t('songs.reanalyzing'):t('songs.reanalyze')}</button></div>}<div className="sheet-toolbar"><label className={view==='original'||!snapshotVerified?'tool-disabled':''}><span>{t('songs.changeKey')}</span><select disabled={view==='original'||!snapshotVerified} value={targetKey||'–'} onChange={(event)=>changeTargetKey(event.target.value)}>{!targetKey&&<option value="–">–</option>}{GERMAN_EDITOR_KEYS.map((key)=><option key={key} value={key}>{displayEditorKeyLabel(key)}</option>)}</select></label><div className={`tool-group simplify-tool${view==='original'||view==='leadsheet'||!snapshotVerified||lyricsOnly?' tool-disabled':''}`}><span>{t('songs.simplifyChords')}</span><button disabled={view==='original'||view==='leadsheet'||!snapshotVerified||lyricsOnly} className={simplifyChords?'active':''} onClick={()=>setSimplifyPref(!simplifyChords)} title={t('songs.simplifyChordsHint')}>{simplifyChords?t('songs.simplifyOn'):t('songs.simplifyOff')}</button></div><div className={`tool-group lyrics-only-tool${view==='original'||view==='leadsheet'||!snapshotVerified?' tool-disabled':''}`}><span>{t('songs.lyricsView')}</span><button disabled={view==='original'||view==='leadsheet'||!snapshotVerified} className={lyricsOnly?'active':''} onClick={()=>setLyricsOnlyPref(!lyricsOnly)} title={t('songs.lyricsViewHint')}>{lyricsOnly?t('songs.lyricsOnlyOn'):t('songs.lyricsOnlyOff')}</button></div><div className={`columns-font-cluster${view==='original'||view==='leadsheet'?' tool-disabled':''}`}><div className={`tool-group${view==='original'||view==='leadsheet'?' tool-disabled':''}`}><span>{t('songs.columns')}</span><button disabled={view==='original'||view==='leadsheet'} className={columns===1?'active':''} onClick={()=>{setColumns(1);persistSheetLayout(1,fontSize)}}>1</button><button disabled={view==='original'||view==='leadsheet'} className={columns===2?'active':''} onClick={()=>{setColumns(2);persistSheetLayout(2,fontSize)}}><Columns2 size={16}/></button></div><div className={`tool-group font-tools${view==='original'||view==='leadsheet'?' tool-disabled':''}`}><span>{t('songs.font')}</span><button disabled={view==='original'||view==='leadsheet'} onClick={()=>{setFontSize((size)=>{const next=Math.max(11,size-1);persistSheetLayout(columns,next);return next})}}>−</button><Type size={18}/><button disabled={view==='original'||view==='leadsheet'} onClick={()=>{setFontSize((size)=>{const next=Math.min(28,size+1);persistSheetLayout(columns,next);return next})}}>+</button><button disabled={view==='original'||view==='leadsheet'} onClick={()=>{setFontSize(16);persistSheetLayout(columns,16)}} title={t('songs.resetFont')}><RotateCcw size={15}/></button></div></div><div className="tool-group scroll-tool"><span>{t('songs.autoScroll')}</span><button className={autoScroll?'active':''} onClick={()=>setAutoScroll((value)=>!value)}>{autoScroll?<Pause size={18}/>:<Play size={18}/>}</button></div><div className="tool-group cajon-tool"><span>{t('songs.cajon')}</span><input aria-label={t('songs.tempoAria')} type="number" min="40" max="240" inputMode="numeric" value={bpmInput} onChange={(event)=>{const raw=event.target.value;setBpmInput(raw);if(raw==='')return;const n=Number(raw);if(Number.isFinite(n))setBpm(n)}} onBlur={()=>{const next=clampTempoBpm(bpmInput,{fallback:null});if(next==null){setBpm(null);setBpmInput('');return}setBpm(next);setBpmInput(String(next))}}/><button className={cajonOn?'active':''} onClick={async ()=>{if(cajonOn){setCajonOn(false);return}playCajonHtmlHit({strong:true});await unlockCajonAudio();await preloadCajonSample();playCajonHit({strong:true});setCajonOn(true)}} title={t('songs.startCajon')}>{cajonOn?<Pause size={18}/>:<Play size={18}/>}</button></div><div className="tool-group sheet-actions"><span>{t('songs.sheet')}</span><button onClick={printSheet} title={t('songs.print')}><Printer size={18}/></button><button onClick={download} title={t('songs.download')}><Download size={18}/></button><button onClick={share} title={t('songs.share')}><Share2 size={18}/></button><button onClick={()=>document.documentElement.requestFullscreen?.()} title={t('songs.fullscreen')}><Maximize2 size={18}/></button></div></div>{view==='original'?<div className="original-pdf-sheet"><AuthorizedFrame title={`${song.title} – ${t('songs.originalPdf')}`} path={originalUrl} hash="#toolbar=0&navpanes=0&view=FitH" songId={song.id} preferPageImages/></div>:view==='leadsheet'?<LeadSheetOsmd songId={song.id} targetKey={targetKey} title={song.title}/>:<><article className="editor-paper"><header><div><h2>{chartTitle}</h2>{song.artist?<p>{song.artist}</p>:null}<p className="chart-header-meta">{[targetKey?`Tonart ${displayEditorKeyLabel(targetKey)}`:null, bpm?`${bpm} BPM`:null].filter(Boolean).join(' · ')||'–'}</p></div><Music2 size={30}/></header><ChartSheet text={text} columns={columns} fontSize={fontSize} editable={snapshotVerified} simplifyChords={simplifyChords} lyricsOnly={lyricsOnly} onCommit={(shown)=>{const knownSource=normalizeEditorKey(sourceKey);const knownTarget=normalizeEditorKey(targetKey);setOverlayText(knownSource&&knownTarget?transposeEditorText(shown,knownTarget,knownSource):shown);setSaved('')}}/></article><div className="editor-bottom-actions">{saved&&<span className="editor-saved"><CheckCircle2 size={16}/>{saved}</span>}<button className="add-button compact" disabled={!snapshotVerified||!text.trim()||saving} onClick={saveEditor}><CheckCircle2 size={18}/>{saving?t('common.saving'):t('songs.saveEdited', { key: targetKey||'–' })}</button></div></>}</>}</section>{tunerOpen&&<GuitarTunerModal onClose={()=>setTunerOpen(false)}/>}</div>
}

  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom">
        ...
        <button className="nav-item account-nav" onClick={()=>navigate('/einstellungen')}>{user.hasPhoto?<img className="account-nav-photo" src={profilePhotoUrl(user)} alt=""/>:<UserRound size={19}/>}<span><b>{user.name}</b><small>{user.role==='admin'?'Administrator':'Mein Songbook'}</small></span></button>
      </div>
    </aside>
        ...
        <aside className="nav-drawer" role="dialog" aria-modal="true" aria-label="Hauptnavigation">
          ...
          <div className="sidebar-bottom">
            ...
            <button className="nav-item account-nav" onClick={()=>go('/einstellungen')}>
              {user.hasPhoto?<img className="account-nav-photo" src={profilePhotoUrl(user)} alt=""/>:<UserRound size={19}/>}
        ...
        <div className="settings-profile-layout">
          <div className="settings-profile-photo">
            <div className="account-photo">
              {user.hasPhoto
                ? <img src={profilePhotoUrl(user)} alt="Profilbild"/>
...
function RunSet({set, songs, onClose}) {
  ...
  return <div className="run-mode"><header><div><p className="eyebrow">Probe läuft · Wischen oder Fußpedal</p><strong>{set.title}</strong><span>{index + 1}/{songs.length} · {song.title}{selectedKey?` · Tonart ${selectedKey}`:''}</span></div><button className="icon-button" onClick={onClose}><X size={22}/></button></header>
    <main className="pdf-stage" onTouchStart={(event) => { touchStart.current = event.touches[0].clientX }} onTouchEnd={(event) => finishSwipe(event.changedTouches[0].clientX)}>
      {selectedKey ? <iframe key={`${song.id}-${selectedKey}`} title={`${song.title} – ${selectedKey}`} src={songChartUrl(song,selectedKey)}/> : hasSongPdf(song) ? <iframe key={song.id} title={song.title} src={`${songPdfUrl(song)}#toolbar=0&navpanes=0&view=FitH`}/> : <div className="no-pdf"><FileText size={42}/><strong>{song.title}</strong><span>Für diesen Song ist keine PDF hinterlegt.</span></div>}

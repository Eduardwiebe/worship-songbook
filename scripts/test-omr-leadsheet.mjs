...
if (process.argv.includes('--full')) {
  ...
  assert(/du warst/.test(rebuilt.text), 'full du warst')
  assert(/Wenn ich auch fliehe, du bist mir nah/.test(rebuilt.text), 'full fliehe line')
  assert(/du wirst verstehn/.test(rebuilt.text), 'full du wirst verstehn')
  assert(/Als wahrer Gott und Mensch warst du hier/.test(rebuilt.text), 'full Als wahrer')
  assert(/ich danke dir/.test(rebuilt.text), 'full ich danke dir')
  assert(/dass du mich kennst und trotzdem liebst/.test(rebuilt.text), 'full refrain clause')
  assert(/mich beim Namen/.test(rebuilt.text), 'full beim Namen')
  assert(/und mir vergibst/.test(rebuilt.text), 'full und mir')
  assert(/mich wieder auf/.test(rebuilt.text), 'full wieder auf')
  assert(/du hebst mich zu dir hinauf/.test(rebuilt.text), 'full hebst mich')
  assert(!/trotzdemliebst|ichdankedir|duhebstmich|Wennichauch/.test(rebuilt.text), 'full no smashed chains')

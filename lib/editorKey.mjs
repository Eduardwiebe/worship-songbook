...
export const GERMAN_EDITOR_KEYS = ['C', 'Cis', 'D', 'Es', 'E', 'F', 'Fis', 'G', 'As', 'A', 'Bb', 'H']
...
const editorSharpNames = ['C', 'Cis', 'D', 'Dis', 'E', 'F', 'Fis', 'G', 'Gis', 'A', 'Ais', 'H']
const editorFlatNames = ['C', 'Des', 'D', 'Es', 'E', 'F', 'Ges', 'G', 'As', 'A', 'B', 'H']
const KEY_ALIASES = {
  'C#': 'Cis', CIS: 'Cis',
  Db: 'Des', DES: 'Des',
  'D#': 'Dis', DIS: 'Dis',
  ...
  Ab: 'As', AS: 'As',
  'A#': 'Ais', AIS: 'Ais',
  // German H = English B. Legacy editor key "B" also meant English B.
  H: 'H',
  B: 'H',
}
...
export const editorChordPattern = /(?<![\p{L}\d])(Cis|Des|Dis|Es(?!us)|Fis|Ges|Gis|As(?!us)|Ais|C#|Db|D#|Eb|F#|Gb|G#|Ab|A#|Bb|[CDEFGABH])((?:m|maj|min|dim|aug|sus|add)?\d*(?:sus\d*)?(?:[#b+°-]\d*)*(?:\/(?:Cis|Des|Dis|Es|Fis|Ges|Gis|As|Ais|C#|Db|D#|Eb|F#|Gb|G#|Ab|A#|Bb|[CDEFGABH]))?)(?![\p{L}\d])/gu
...
export function inferKeyFromLeadsheet(text) {
  ...
  const keyToken = '(Cis|Des|Dis|Es(?!us)|Fis|Ges|Gis|As(?!us)|Ais|C#|Db|D#|Eb|F#|Gb|G#|Ab|A#|Bb|[CDEFGABH])'
  const patterns = [
 ...
 * Explicit document headers (TONART / "Key - X") are authoritative — SongSelect
 * charts often include borrowed chords (Bb in C, bVII in E) that fool diatonic
 * scoring. Vision-only conflicts still prefer chords + review (legacy).
 */
  ...
  if (explicit && chords.key && explicit !== chords.key) {
    if (tonart) {
      return {
        ...
        confidence: 0.95,
        method: 'explicit_document_key_over_chord_conflict',
        needsReview: false,
        visionKey: vision,
        tonartKey: tonart,
        chordKey: chords.key,
...
/**
 * Worship-band chord simplify (display/projection layer).
 * Rules:
 * - maj7 / maj9 / maj11 / maj13 → major triad
 * - m7 / min7 / m9… and plain 7 / 9 / 11 / 13 / add* → triad or minor triad
 * - keep sus (optionally) as sus / sus2 / sus4
 * - keep dim / aug / ° as-is (quality matters)
 * - drop slash bass by default (E/Gis → E)
 * German roots are applied by transpose; this only collapses suffixes.
 */
export function simplifyChordSuffix(suffix, { keepSus = true, dropSlash = true } = {}) {
  let rest = String(suffix || '')
  let bass = ''
  const slash = rest.match(/\/(Cis|Des|Dis|Es(?!us)|Fis|Ges|Gis|As(?!us)|Ais|C#|Db|D#|Eb|F#|Gb|G#|Ab|A#|Bb|[CDEFGABH])$/)
  if (slash) {
    bass = dropSlash ? '' : slash[0]
    rest = rest.slice(0, -slash[0].length)
  }
  const susMatch = rest.match(/sus\d*/i)
  if (keepSus && susMatch) {
    const sus = susMatch[0].toLowerCase()
    return `${sus === 'sus' ? 'sus' : sus}${bass}`
  }
  // Quality-preserving bases for worship-band play
  if (/dim|°/.test(rest)) return `dim${bass}`
  if (/^aug|\+/i.test(rest) && !/add/i.test(rest)) return `aug${bass}`
  if (/^ø/.test(rest)) return `dim${bass}`
  // Minor family → m (m7, min7, m9, m11, m13, …)
  if (/^(m|min)(?!aj)/i.test(rest)) return `m${bass}`
  // Everything else with extensions (maj7, 7, add2, 9, …) → major triad
  return `${bass}`
}

export function simplifyChordToken(chord, options = {}) {
  const raw = String(chord || '').trim()
  if (!raw) return raw
  const match = raw.match(/^(Cis|Des|Dis|Es(?!us)|Fis|Ges|Gis|As(?!us)|Ais|C#|Db|D#|Eb|F#|Gb|G#|Ab|A#|Bb|[CDEFGABH])(.*)$/)
  if (!match) return raw
  return match[1] + simplifyChordSuffix(match[2], options)
}

export function simplifyEditorText(text, options = {}) {
  return String(text || '').split('\n').map((line) => {
    if (!isEditorChordLine(line)) return line
    const matches = editorChordTokens(line)
    ...
    for (let index = 0; index < matches.length; index += 1) {
      ...
      const simplified = simplifyChordToken(match[0], options)
      output += simplified
...
export function formatEditorDisplayText(text, { simplify = true, ...simplifyOptions } = {}) {
  return simplify ? simplifyEditorText(text, simplifyOptions) : String(text || '')
}

/** Always original chords + (source → selected). Never re-transpose the current view. */
/** Rewrite chord roots to German/European spelling without changing pitch. */
export function respellEditorTextGerman(text, targetKey = 'C') {
  const target = normalizeEditorKey(targetKey) || 'C'
  return String(text || '').split('\n').map((line) => {
    if (!isEditorChordLine(line)) return line
    const matches = editorChordTokens(line)
    if (!matches.length) return line
    ...
    for (let index = 0; index < matches.length; index += 1) {
      ...
      const desiredColumn = match.index
      if (output.length < desiredColumn) output += ' '.repeat(desiredColumn - output.length)
      const respelled = transposeEditorChordToken(match[1], match[2], 0, target)
      output += respelled
      const originalEnd = match.index + match[0].length
      const next = matches[index + 1]
      const segmentEnd = next ? next.index : line.length
...
export function displayEditorText(originalText, sourceKey, targetKey, {
  ...
  if (source && target && source !== target) text = transposeEditorText(originalText, source, target)
  else if (germanNotation && target) text = respellEditorTextGerman(text, target)
  else if (germanNotation && source) text = respellEditorTextGerman(text, source)
  return simplify ? simplifyEditorText(text, simplifyOptions) : text
}

/** UI label for a stored editor key (Bb → B for German/European spelling). */
export function displayEditorKeyLabel(key) {
  const normalized = normalizeEditorKey(key)
  if (normalized === 'Bb') return 'B'
  return normalized || String(key || '')
}

/**
 * Pitch class name for transpose output — always German/European spelling:
 * H for English B, B for Bb, Cis/Dis/Fis/Gis/As/Es/Des/Ges (no # / Bb).
 */
export function editorPitchName(idx, targetKey) {
  const useFlat = ['F', 'Bb', 'B', 'Es', 'As', 'Des', 'Ges'].includes(targetKey)
  if (idx === 10 && (useFlat || ['C', 'G', 'D'].includes(targetKey))) return 'B'
...
function transposeEditorChordToken(note, suffix, shift, targetKey) {
  const slash = suffix.match(/\/(Cis|Des|Dis|Es(?!us)|Fis|Ges|Gis|As(?!us)|Ais|C#|Db|D#|Eb|F#|Gb|G#|Ab|A#|Bb|[CDEFGABH])$/)

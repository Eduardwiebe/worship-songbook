...
import { placeChordsByIndex as packChordsByIndex, collapseGluedSlashBassAnchors, softFormatChordChart } from './chartLayout.mjs'
  ...
  for (const section of Array.isArray(source.sections) ? source.sections : []) {
    ...
    for (const line of Array.isArray(section?.lines) ? section.lines : []) {
      ...
      for (const item of Array.isArray(line?.chords) ? line.chords : []) {
        ...
        const base = Math.max(0, Math.min(lyrics.length, Number(item?.index) || 0))
        parts.forEach((chord, offset) => {
          chords.push({ chord, index: Math.min(lyrics.length, base + offset) })
        })
      }

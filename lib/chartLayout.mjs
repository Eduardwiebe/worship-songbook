...
import { mergeSyllabicLyricLine, prepareChordViewStructure } from './chordViewText.mjs'
import { expandStackedVerseText } from './songSelectStructure.mjs'
...
function mapAnchorIndex(map, index) {
  if (!map?.length) return Math.max(0, index || 0)
  const clamped = Math.max(0, Math.min(index || 0, map.length - 1))
  return map[clamped] ?? 0
}

function scaleAnchorIndex(index, fromLen, toLen) {
  if (toLen <= 1) return 0
  const src = Math.max(fromLen, 1)
  return Math.max(0, Math.min(toLen - 1, Math.round((index / src) * toLen)))
}

function isMergeableLyric(line) {
  const text = String(line || '')
  if (!text.trim()) return false
  if (isChordLine(text) || isChordLine(text.trim())) return false
  if (isChartSectionLine(text) || isChartMetaLine(text)) return false
  return true
}

/**
 * Merge syllabic lyric lines and keep chord columns on the merged words.
 * A second lyric under the same chord row (SongSelect verse 1 + 2) gets
 * the same chords, scaled onto that line. LeadSheet/MusicXML does not use this.
 */
export function mergeSyllablesInChartLines(lines) {
  const out = []
  for (let i = 0; i < lines.length; i += 1) {
    const line = lines[i]
    const next = lines[i + 1]
    const chordish = isChordLine(line) || isChordLine(String(line || '').trim())
    if (chordish && isMergeableLyric(next)) {
      const lyrics = [next]
      let j = i + 2
      const chordCount = anchorsFromChordLine(line).length
      while (
        j < lines.length
        && lyrics.length < 3
        && chordCount >= 2
        && isMergeableLyric(lines[j])
        && String(lines[j]).trim().split(/\s+/).length >= 3
      ) {
        lyrics.push(lines[j])
        j += 1
      }
      const anchors = anchorsFromChordLine(line)
      const first = mergeSyllabicLyricLine(lyrics[0])
      if (!first.changed) {
        out.push(line)
        out.push(lyrics[0])
      } else {
        const packed = packChordsAboveLyrics(first.text, anchors.map((anchor) => ({
          chord: anchor.chord,
          index: mapAnchorIndex(first.map, anchor.index),
        })))
        if (packed.chordLine.trim()) out.push(packed.chordLine)
        out.push(packed.lyricLine)
      }
      for (let n = 1; n < lyrics.length; n += 1) {
        const merged = mergeSyllabicLyricLine(lyrics[n])
        const scaled = packChordsAboveLyrics(merged.text, anchors.map((anchor) => ({
          chord: anchor.chord,
          index: mapAnchorIndex(merged.map, scaleAnchorIndex(anchor.index, String(lyrics[0]).length, String(lyrics[n]).length)),
        })))
        if (scaled.chordLine.trim()) out.push(scaled.chordLine)
        out.push(scaled.lyricLine)
      }
      i = j - 1
      continue
    }
    if (isMergeableLyric(line)) {
      out.push(mergeSyllabicLyricLine(line).text)
      continue
    }
    out.push(line)
  }
...
export function softFormatChordChart(text) {
  const expanded = expandStackedVerseText(text)
  const lines = mergeSyllablesInChartLines(prepareChordViewStructure(expanded || text))
  const out = []

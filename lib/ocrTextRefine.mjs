...
const FUNCTION = new Set([
  ...
  'für', 'fur', 'war', 'ist', 'uns', 'wir', 'ihr', 'sie',
...
function segmentWord(word, { confidence = 1 } = {}) {
  ...
  const chosen = best[n]
  const hasFunction = chosen?.kinds.includes('function')
  const margin = confidence >= 0.85 ? 28 : 18
  if (!chosen || !hasFunction || chosen.parts.length < 2 || chosen.score < whole.score + margin) {

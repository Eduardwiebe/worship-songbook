...
import { softFormatChordChart, deinterleaveTwoColumnLayout } from './chartLayout.mjs'
...
export function parseChordOverLyricsText(raw, { titleHint = '', injectTonart = true } = {}) {
  const cleaned = cleanOcrText(deinterleaveTwoColumnLayout(raw))
  ...
  let text = softFormatChordChart(out.join('\n').replace(/\n{3,}/g, '\n\n').trim())
...
export function analyzePdfPageText(pageText) {
  const cleaned = cleanOcrText(deinterleaveTwoColumnLayout(pageText))

...
const IMAGE_EXT = /\.(jpe?g|png|gif|webp|heic|heif|bmp|tif|tiff)$/i
const PDF_EXT = /\.pdf$/i
const TXT_EXT = /\.txt$/i
const GENERIC_IOS_NAME = /^(image|img|photo|scan|screenshot|fullsizerender)([_\s.-]|$)/i
 ...
 * @param {{ assumeImage?: boolean }} [options] assumeImage: picker was camera/gallery
 */
export function isLikelyScanImageFile(file, { assumeImage = false } = {}) {
  ...
  const unknownType = !type || type === 'application/octet-stream' || type === 'application/x-octet-stream'
  if (!unknownType) return false
  if (assumeImage) return true
  if (!name) return true
  if (!name.includes('.') || GENERIC_IOS_NAME.test(name)) return true
  return false
}
...
export function titleFromScanFile(file) {
  const stripped = fileNameOf(file).replace(/\.[^.]+$/, '').trim()
  if (!stripped || GENERIC_IOS_NAME.test(stripped)) return ''
  return stripped
}
...
export function resolveScanTitle(title, fallbackFile) {
  ...
  const fromFile = titleFromScanFile(fallbackFile)
  if (fromFile) return fromFile
  return 'Scan'
}

/**
 * Enable "Scannen und Lead-Sheet erstellen" once a source exists.
 * Title is optional for image/PDF pages (resolved on submit); required for paste-text.
 */
export function canSubmitScan({
  ...
  if (mode === 'text') return Boolean(String(title || '').trim() && String(pasteText || '').trim())

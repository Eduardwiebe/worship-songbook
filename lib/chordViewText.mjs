...
/**
 * Join a wrapped syllable onto the previous word when the concatenation
 * is a real word ("er" + "löst" → "erlöst"). Right-hand capitalized words stay put.
 * @returns {string|null}
 */
export function lexiconJoinedWord(leftRaw, rightRaw) {
  const left = core(leftRaw)
  const right = core(rightRaw)
  if (!left || !right) return null
  if (/^\p{Lu}/u.test(String(rightRaw || '').trim()) && isStandaloneLyricWord(rightRaw)) return null
  if (!JOIN_LEXICON.has(left + right)) return null
  const punct = (String(rightRaw || '').match(/[.,:;!?]+$/u) || [''])[0]
  const leftLetters = String(leftRaw || '').replace(/[.,:;!?]+$/u, '')
  const rightLetters = String(rightRaw || '').replace(/[.,:;!?]+$/u, '')
  return `${leftLetters}${rightLetters}${punct}`
}
...
function shouldMergeSlice(tokens) {
  ...
  if (JOIN_LEXICON.has(joined)) {
    ...
    // "herr schen" → herrschen: both pieces can look like words, but the
    // concatenation is the real word and the left piece is only a syllable.
    if (tokens.length === 2 && core(tokens[0].raw).length <= 4) return true

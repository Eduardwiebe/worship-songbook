...
import { chordPattern, cleanOcrText, isChordLine, pitchMap, scoreLeadsheetQuality, parseTempoBpm } from './leadsheetAnalysis.mjs'
import { packChordsAboveLyrics, softFormatChordChart, deinterleaveTwoColumnLayout } from './chartLayout.mjs'
import { expandSongSelectFromTokens, expandStackedVerseText } from './songSelectStructure.mjs'
import { isClosedClassWord, isProtectedWholeWord, normalizeChordGlyphs, refineLyricText } from './ocrTextRefine.mjs'
import { extractSongMetadata, filterMetadataLines, isMetadataText, keyFromLeadSheetFilename, preferSongTitle } from './songMetadata.mjs'
import { collectNativeLexicon, rehydrateLyricTokens } from './syllableRehydrate.mjs'
import { buildLeadSheetMusicXml, classifyChordPlacement, musicXmlHasMelody } from './musicxmlLeadSheet.mjs'
...
const COPYRIGHT_RE = /(?:©|copyright|\bcc\s*li\b|songselect|verlag|hänssler|hanssler|immanuel\s+music|nach\s+psalm|\bmelodie\b|\btext\s*\(|all rights reserved|terms of use)/i
...
let activeNativeTokens = []
let activeLexicon = null

function withNativeLyricContext(nativeTokens, fn) {
  const previousTokens = activeNativeTokens
  const previousLexicon = activeLexicon
  activeNativeTokens = Array.isArray(nativeTokens) ? nativeTokens : []
  activeLexicon = collectNativeLexicon(activeNativeTokens)
  try {
    return fn()
  } finally {
    activeNativeTokens = previousTokens
    activeLexicon = previousLexicon
  }
}
...
function isCopyrightText(text) {
  const value = String(text || '').trim()
  return COPYRIGHT_RE.test(value) || YEAR_PUB_RE.test(value) || isMetadataText(value)
}
...
function classifyToken(token) {
  ...
  if (isMetadataText(text)) return 'meta'
  ...
  if (isPageOrSongNumber(text) && (token.region === 'header' || token.region === 'footer' || (token.bbox?.[0] ?? 0) < 120)) {
    return (token.bbox?.[0] ?? 0) < 120 && token.region === 'lyric' ? 'verse_number' : 'meta'
  }
...
export function joinLyricTokens(tokens) {
  ...
  if (usable.some((token) => Array.isArray(token.bbox) && token.bbox.length >= 4)) {
    const rehydrated = rehydrateLyricTokens(usable, {
      nativeTokens: activeNativeTokens,
      lexicon: activeLexicon,
    })
    if (rehydrated) return refineLyricText(normalizeEngravedLyrics(rehydrated)).text
  }
...
function detectTitle(headerTokens, titleHint, filename = '') {
  const native = (() => {
    ...
    for (const line of lines) {
      ...
      if (!text || isRubricText(text) || isCopyrightText(text) || isMetadataText(text)) continue
      const avgH = line.tokens.reduce((sum, token) => sum + tokenHeight(token), 0) / Math.max(line.tokens.length, 1)
      if (text.length <= 70 && avgH >= 14) return text
    }
    return ''
  })()
  return preferSongTitle({ nativeTitle: native, filename, hint: titleHint })
}
...
export function reconstructLeadsheet(structured, { titleHint = '', filename = '', nativeTokens = [] } = {}) {
  return withNativeLyricContext([
    ...nativeTokens,
    ...flattenTokens(structured).all.filter((token) => token.source === 'pdftotext-bbox' || token.role === 'Title'),
  ], () => reconstructChordViewDocument(structured, { titleHint, filename }))
}

function reconstructChordViewDocument(structured, { titleHint = '', filename = '' } = {}) {
  ...
  const title = detectTitle(header, titleHint, filename)
  const metadata = extractSongMetadata([...header, ...all.filter((token) => classifyToken(token) === 'meta')], {
    filename,
    titleHint,
  })
  if (title) metadata.title = title
  const tempo = parseTempoBpm(all.map((token) => token.text).join(' '))
  if (tempo) metadata.bpm = tempo
  ...
  for (const block of expanded.blocks) {
    ...
    } else if (block.type === 'lyrics' && isMetadataText(block.text)) {
      continue
    } else {
      body.push(block.text)
    }
  ...
  text = filterMetadataLines(cleanOcrText(text))
  ...
  const chordReview = systems.some((system) => (system.chords || []).some((chord) => {
    const notes = (structured?.pages || []).flatMap((page) => page.notes || [])
    if (!notes.length) return false
    return classifyChordPlacement(chord, notes).review
  }))
  const reviewReasons = [
    ...
    chordReview ? 'chord_measure_only' : '',
  ].filter(Boolean)
...
/**
 * TWO reconstructions from the same hybrid OMR + native PDF input:
 * 1) chordView — readable song text (rehydrated words, metadata filtered)
 * 2) leadSheet — MusicXML notation (syllabic lyrics under notes, chords above staff)
 */
export function reconstructHybridSong(structured, {
  titleHint = '',
  filename = '',
  nativeTokens = [],
  nativeTitle = '',
} = {}) {
  const chordView = reconstructLeadsheet(structured, { titleHint, filename, nativeTokens })
  const songSelect = expandSongSelectFromTokens(nativeTokens)
  if (songSelect?.text) {
    chordView.text = songSelect.text
    chordView.quality = scoreLeadsheetQuality(songSelect.text)
    chordView.needsReview = chordView.quality.needsReview
    if (songSelect.title) chordView.title = songSelect.title
    if (songSelect.bpm) {
      chordView.metadata = { ...(chordView.metadata || {}), bpm: songSelect.bpm }
    }
    if (chordView.layout) chordView.layout.repeatedRefrain = true
  }
  const title = preferSongTitle({
    nativeTitle: nativeTitle || chordView.title,
    filename,
    hint: titleHint,
  })
  const key = chordView.metadata?.key || keyFromLeadSheetFilename(filename) || ''
  if (key && chordView.metadata && !chordView.metadata.key) chordView.metadata.key = key
  const pages = structured?.pages || []
  const notes = pages.flatMap((page) => page.notes || [])
  const sourceMusicXml = structured?.musicxml || pages.map((page) => page.musicxml).find(Boolean) || ''
  const chords = pages.flatMap((page) => (page.tokens || []).filter((token) => {
    const kind = classifyToken(token)
    return kind === 'chord' || token.source === 'audiveris-chord' || (token.source === 'pdftotext-bbox' && isValidChordToken(token.text))
  })).map((token) => ({
    ...token,
    text: normalizeChordGlyphs(token.text),
  })).filter((token) => isValidChordToken(token.text))

  const lyricTokens = pages.flatMap((page) => (page.tokens || []).filter((token) => {
    const kind = classifyToken({ ...token, region: token.region || 'lyric' })
    return kind === 'lyric' || kind === 'syllable'
  }))
  const linkage = lyricsToHomrNoteVerified(lyricTokens, notes)
  const musicxml = (notes.length || musicXmlHasMelody(sourceMusicXml))
    ? buildLeadSheetMusicXml({
      title,
      key,
      notes,
      chords,
  ...
  return {
    chordView: {
      ...chordView,
      title,
      text: title && chordView.text && !chordView.text.toLowerCase().startsWith(title.toLowerCase())
        ? `${title}\n\n${filterMetadataLines(chordView.text)}`
        : filterMetadataLines(chordView.text),
    },
    leadSheet: {
      musicxml,
      hasMelody: musicXmlHasMelody(musicxml),
      lyricNoteLinks: linkage,
      engine: structured?.engine || 'unknown',
    },
    metadata: {
      ...chordView.metadata,
...
function packColumnFromBBoxTokens(tokens, { titleHint = '' } = {}) {
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    if (isMetadataText(joined) || row.tokens.every((token) => isMetadataText(token.text) || classifyToken(token) === 'meta')) {
      continue
    }
...
export function reconstructFromPdfBBox(xml, { titleHint = '', filename = '' } = {}) {
  ...
  if (!structured.pages.length || !structured.pages.some((p) => p.tokens?.length)) {
    return {
      ...
      needsReview: true,
      metadata: extractSongMetadata('', { filename, titleHint }),
    }
  }

  const allTokens = (parsed.pages || []).flatMap((page) => page.tokens || [])
  const songSelect = expandSongSelectFromTokens(allTokens)
  return withNativeLyricContext(allTokens, () => {
    ...
    const titleToken = top.find((token) => String(token.text || '').length > 8 && tokenCenterY(token) < height * 0.07 && !isMetadataText(token.text))
    const nativeTitle = titleToken ? joinLyricTokens(top.filter((token) => Math.abs(tokenCenterY(token) - tokenCenterY(titleToken)) < 8 && !isMetadataText(token.text))) : ''
    ...
    const bodies = songSelect?.text
      ? [songSelect.text]
      : structured.pages.map((page) => packColumnFromBBoxTokens(page.tokens || [], { titleHint }))
    let text = filterMetadataLines(bodies.filter(Boolean).join('\n\n').replace(/\n{3,}/g, '\n\n').trim())
    const header = []
    const alreadyTitled = title && text.toLowerCase().startsWith(title.toLowerCase())
    if (title && !alreadyTitled) header.push(title)
    if (metaParts.length) header.push(metaParts.join(' | '))
    if (header.length) text = `${header.join('\n\n')}\n\n${text}`.replace(/\n{3,}/g, '\n\n').trim()
    text = filterMetadataLines(softFormatChordChart(text))
    const quality = scoreLeadsheetQuality(text)
    const metadata = extractSongMetadata(allTokens, { filename, titleHint })
    metadata.title = title
    if (songSelect?.bpm) metadata.bpm = songSelect.bpm
    else if (tempoMatch) metadata.bpm = Number(tempoMatch[1])
...
export function reconstructFromFlatText(text) {
  ...
  const stacked = expandStackedVerseText(raw)
  const cleaned = softFormatChordChart(stacked || deinterleaveTwoColumnLayout(cleanOcrText(raw)))

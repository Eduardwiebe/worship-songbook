...
export function evaluateScanSnapshotVerification(result = {}) {
  ...
  // Key agreement is authoritative. Stale analysis needsReview (e.g. set before
  // TONART injection) must not block a snapshot when document/vision key and
  // chord analysis already agree on the same original key.
  ...
  const verified = Boolean(keysAgree && chordModel?.lines?.length)
  return {
    text,
    visionKey,
    documentKey,
...
export function persistScanSnapshot(db, { songId, pdfPath, documentHash, result, createdAt = new Date().toISOString() }) {
  ...
  const analysisMetadata = {
    ...
    sameRun: true,
    verificationRule: 'resolved_key_and_chord_analysis_agree',
  }
  db.exec('BEGIN')
  try {

...
function chordsConfidence(text, chordKey) {
  if (!chordKey) return 0
  return inferKeyFromChords(text).confidence || 0
}
export function evaluateScanSnapshotVerification(result = {}) {
  ...
  const resolved = resolveScanSourceKey({
    visionKey,
    text,
    visionConfidence: result?.avgConfidence,
  })
  // Key agreement is authoritative when chords and resolved key match.
  // Stale analysis needsReview must not block when they already agree.
  // Explicit SongSelect/TONART headers also verify even if borrowed chords
  // make diatonic scoring pick a different key (Bb→F, bVII→A, …).
  const keysAgree = Boolean(
    text
    && resolved.key
    && chordKey
    && resolved.key === chordKey
    && !resolved.needsReview
  )
  const documentKeyTrusted = Boolean(
    text
    && resolved.key
    && documentKey
    && resolved.key === documentKey
    && !resolved.needsReview
  )
  const chordModel = buildOriginalChordModel(text)
  const verified = Boolean((keysAgree || documentKeyTrusted) && chordModel?.lines?.length)
  return {
    text,
    visionKey,
    documentKey,
...
export function persistScanSnapshot(db, { songId, pdfPath, documentHash, result, createdAt = new Date().toISOString() }) {
  ...
  const analysisMetadata = {
    ...
    chordConfidence: chordsConfidence(text, chordKey),
    analysisConfidence: result?.avgConfidence ?? null,
    analysisNeedsReview: Boolean(result?.needsReview),
    resolvedNeedsReview: Boolean(resolved.needsReview),
    sameRun: true,
    verificationRule: evaluation.verified
      ? (documentKey && resolved.key === documentKey && resolved.key !== chordKey
        ? 'explicit_document_key_trusted'
        : 'resolved_key_and_chord_analysis_agree')
      : 'resolved_key_and_chord_analysis_agree',
  }
  db.exec('BEGIN')
  try {
    const ids = insertSnapshotAndEvidence(db, {
      ...
      analysisMetadata,
      confidence: verified ? Math.min(Number(result?.avgConfidence) || 1, chordsConfidence(text, chordKey) || 1) : 0,
      createdAt,
    })
    if (verified) {
      db.prepare(`
...
export function saveVariantFromVerifiedSnapshot(db, songId, payload, { createdAt = new Date().toISOString() } = {}) {
  ...
  try {
    ...
    if (payload?.sheetColumns != null || payload?.sheetFontSize != null) {
      const current = db.prepare('SELECT sheet_columns, sheet_font_size FROM songs WHERE id=?').get(songId) || {}
      const cols = payload?.sheetColumns != null
        ? (Number(payload.sheetColumns) === 2 ? 2 : 1)
        : (Number(current.sheet_columns) === 2 ? 2 : 1)
      const font = payload?.sheetFontSize != null
        ? Math.min(28, Math.max(11, Number(payload.sheetFontSize) || 16))
        : Math.min(28, Math.max(11, Number(current.sheet_font_size) || 16))
      db.prepare('UPDATE songs SET sheet_columns=?, sheet_font_size=? WHERE id=?').run(cols, font, songId)
    }
    db.exec('COMMIT')
  } catch (error) {
    db.exec('ROLLBACK')
  ...
  const layout = db.prepare('SELECT sheet_columns AS sheetColumns, sheet_font_size AS sheetFontSize FROM songs WHERE id=?').get(songId) || {}
  return {
    targetKey,
    sourceKey: snapshot.sourceKey,
    ...
    sheetColumns: Number(layout.sheetColumns) === 2 ? 2 : 1,
    sheetFontSize: Math.min(28, Math.max(11, Number(layout.sheetFontSize) || 16)),
  }
}


export function loadLatestSongSnapshotRow(db, songId) {
  const row = db.prepare(`
    SELECT * FROM song_original_snapshots
    WHERE song_id=?
    ORDER BY CASE status WHEN 'verified' THEN 0 WHEN 'review_required' THEN 1 ELSE 2 END, created_at DESC, id DESC
    LIMIT 1
  `).get(songId)
...
/**
 * Soft-repair: if a stored lead-sheet text already has agreeing original key
 * evidence, append a verified snapshot without re-running OCR/vision.
 */
export function repairSongSnapshotFromStoredText(db, songId, {
  pdfPath = '',
  documentHash = '',
  createdAt = new Date().toISOString(),
} = {}) {
  if (getVerifiedSongSnapshot(db, songId)) {
    return { repaired: false, reason: 'already_verified', state: getSongSnapshotState(db, songId) }
  }
  const existing = loadLatestSongSnapshotRow(db, songId)
  const text = String(existing?.originalText || '')
  if (!text.trim()) {
    return { repaired: false, reason: 'no_stored_text', state: getSongSnapshotState(db, songId) }
  }
  const evaluation = evaluateScanSnapshotVerification({
    text,
    method: 'stored_text_repair',
    needsReview: false,
    key: inferKeyFromLeadsheet(text),
  })
  if (!evaluation.verified) {
    return { repaired: false, reason: 'still_unverified', evaluation, state: getSongSnapshotState(db, songId) }
  }
  const song = db.prepare('SELECT pdf_path FROM songs WHERE id=?').get(songId)
  const path = pdfPath || song?.pdf_path || existing?.provenanceRef || `song:${songId}`
  const hash = documentHash || sha256(text)
  persistScanSnapshot(db, {
    ...
    result: {
      text,
      key: evaluation.detectedKey,
      method: 'stored_text_repair',
      needsReview: false,
      avgConfidence: evaluation.resolved.confidence || 1,
    },
    createdAt,
  })
  return { repaired: true, reason: 'soft_repaired', state: getSongSnapshotState(db, songId) }
}

/**
 * Replace any existing verified snapshot so a fresh analysis can become the
 * sole verified original (unique partial index on verified song_id).
 */
export function clearVerifiedSongSnapshot(db, songId) {
  const verified = db.prepare(`
    SELECT id FROM song_original_snapshots
    WHERE song_id=? AND status='verified'
  `).all(songId)
  if (!verified.length) return 0
  // Snapshots/evidence are append-only via triggers; reanalyze must briefly
  // drop those delete guards so the unique verified index can accept a replacement.
  db.exec(`
    DROP TRIGGER IF EXISTS song_original_snapshots_no_delete;
    DROP TRIGGER IF EXISTS song_original_snapshots_no_update;
    DROP TRIGGER IF EXISTS song_evidence_no_delete;
    DROP TRIGGER IF EXISTS song_evidence_no_update;
  `)
  try {
    for (const row of verified) {
      db.prepare('DELETE FROM song_variants WHERE song_id=? AND snapshot_id=?').run(songId, row.id)
      db.prepare('DELETE FROM song_evidence WHERE snapshot_id=?').run(row.id)
      db.prepare('DELETE FROM song_original_snapshots WHERE id=?').run(row.id)
    }
  } finally {
    db.exec(`
      CREATE TRIGGER IF NOT EXISTS song_original_snapshots_no_update
...
export function replaceScanSnapshot(db, { songId, pdfPath, documentHash, result, createdAt = new Date().toISOString() }) {
  clearVerifiedSongSnapshot(db, songId)
  return persistScanSnapshot(db, { songId, pdfPath, documentHash, result, createdAt })
}
